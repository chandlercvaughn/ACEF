...
# E402: these imports must follow the sys.path insert above — the script is
# runnable from a checkout without an installed package.
import acef  # noqa: E402
from acef.package import Package  # noqa: E402
...
TS_NOW = datetime(2026, 3, 15, 12, 0, 0, tzinfo=UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
TS_RECENT = datetime(2026, 3, 1, 10, 0, 0, tzinfo=UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
TS_OLD = datetime(2024, 1, 1, 0, 0, 0, tzinfo=UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def build_eu_high_risk_core() -> None:
    ...
    pkg.record(
        ...
        payload={
            ...
            "description": (
                "Potential misdiagnosis due to distribution shift in patient demographics between training data and "
                "deployment population, leading to reduced accuracy for underrepresented groups"
            ),
            "category": "safety",
            "likelihood": "possible",
            "severity": "major",
            ...
            "residual_risk_justification": (
                "Demographic-aware recalibration layer reduces differential error rates to below 3% across all "
                "subgroups. Continuous monitoring dashboard deployed."
            ),
    ...
    pkg.record(
        ...
        payload={
            ...
            "control_description": (
                "Demographic-aware recalibration layer that adjusts model confidence thresholds based on patient "
                "demographics. Combined with a safety guardrail that flags low-confidence predictions for mandatory "
                "human review."
            ),
            ...
            "effectiveness_assessment": (
                "Recalibration reduces differential error rates from 8.2% to 2.7% across demographic subgroups. Safety "
                "guardrail captures 99.1% of cases where recalibrated confidence is below threshold."
            ),
    ...
    pkg.record(
        ...
        payload={
            ...
            "collection_methodology": (
                "Electronic health records from Beth Israel Deaconess Medical Center, de-identified per HIPAA Safe "
                "Harbor"
            ),
            "collection_date_range": {"start": "2008-01-01", "end": "2022-12-31"},
            "representativeness_assessment": (
                "Dataset covers 524k patient admissions with broad demographic representation. Known "
                "underrepresentation of pediatric (<5%) and geriatric (>90 years, <2%) populations."
            ),
            ...
            "quality_checks_performed": [
                ...
                {
                    "check_name": "missing_value_analysis",
                    "result": "4.1% missing values imputed using clinical rules",
                    "date": "2025-03-20",
                },
            ],
            "bias_mitigation_methods": [
                "Demographic stratification analysis",
                "Synthetic augmentation for underrepresented cohorts",
            ],
            ...
            "chain_of_custody": [
                {
                    ...
                    "actor": "HealthAI Corp Data Engineering",
                    "date": "2025-02-01",
                    "action": "received",
                },
                {
                    "step": "De-identification verification",
                    "actor": "HealthAI Privacy Team",
                    "date": "2025-02-10",
                    "action": "processed",
                },
            ],
    ...
    pkg.record(
        ...
        payload={
            "methodology": (
                "Stratified k-fold cross-validation (k=5) with demographic subgroup analysis, supplemented by "
                "prospective validation on 3-month held-out deployment data"
            ),
    ...
    pkg.record(
        ...
        payload={
            ...
            "traceability_rationale": (
                "Logging captures all inference events with input/output hash commitments enabling traceability "
                "without "
                "storing patient data."
            ),
            "retention_policy_summary": {
                # Art. 12 states NO retention period (three paragraphs, none
                # carrying a duration). Provider-held logs are governed by
                # Art. 19(1) at "at least six months"; 365 days is a lawful
                # provider choice above that floor. The previous 3650 /
                # Art. 12(1) pairing published the exact misattribution that
                # GitHub issue #1 reported.
                "min_days": 365,
    ...
    pkg.record(
        ...
        payload={
            ...
            "correlation_id": "INF-2026-03-01-00142",
            "reason": (
                "Model confidence below clinical threshold (0.72) for differential diagnosis of rare cardiac "
                "arrhythmia."
            ),
            ...
            "impact_assessment": (
                "No patient harm. Additional diagnostics confirmed clinician override was appropriate."
            ),
    ...
    pkg.record(
        ...
        payload={
            ...
            "capabilities": [
                "Clinical diagnosis suggestion",
                "Differential diagnosis ranking",
                "Drug interaction screening",
            ],
            "limitations": [
                "Not validated for pediatric patients under 18",
                "Performance degrades on non-English clinical notes",
            ],
...
def build_gpai_provider_annex_xi_xii() -> None:
    ...
    training_dataset = pkg.add_dataset(
        ...
        source_type="scraped",
        modality="text",
        size={"records": 15000000000, "size_gb": 8500.0},
        subject_refs=[model.subject_id],
    )
    licensed_dataset = pkg.add_dataset(
        "Licensed-News-Archive",
        version="3.1",
        source_type="licensed",
        modality="text",
        size={"records": 850000000, "size_gb": 420.0},
        subject_refs=[model.subject_id],
    )
    ...
    pkg.record(
        ...
        payload={
            ...
            "methodology": (
                "Comprehensive evaluation per Annex XI covering model architecture, training data, compute resources, "
                "evaluation benchmarks, and known limitations"
            ),
...
def build_synthetic_content_marking() -> None:
    ...
    pkg.record(
        ...
        payload={
            ...
            "watermark_family": "spectral_embedding",
            "jurisdiction": "EU",
        ...
        entity_refs=refs,
        lifecycle_phase="deployment",
        collector={"name": "contentgen-marking-service", "version": "5.0.0"},
        timestamp=TS_RECENT,
    )
    ...
    pkg.record(
        ...
        payload={
            ...
            "metadata_container": "xmp/c2pa-manifest-store",
            "watermark_applied": True,
            "watermark_family": "robust-imperceptible",
            "jurisdiction": "EU",
            "marking_technique": "secure_metadata",
        },
        obligation_role="provider",
    ...
    pkg.record(
        ...
        provisions=["article-50.2"],
        payload={
            "disclosure_subtype": "deepfake",
            "label_type": "deepfake_disclosure",
            "presentation": "visible_icon_plus_text",
            "locale": "en-US",
        ...
        entity_refs=refs,
        lifecycle_phase="deployment",
        collector={"name": "contentgen-disclosure-service", "version": "5.0.0"},
        timestamp=TS_RECENT,
    )
    ...
    pkg.record(
        ...
        payload={
            "event_type": "marking",
            "correlation_id": "MRK-2026-03-01-00001",
            "session_id": "GEN-SES-20260301-00042",
    ...
    pkg.record(
        ...
        payload={
            ...
            "methodology": (
                "Automated detectability testing using adversarial transformations on 10,000 marked images and 1,000 "
                "marked "
                "videos"
            ),
            ...
            "robustness_results": {
                "jpeg_compression_q40": 0.968,
                "crop_30_percent": 0.951,
                "social_media_recompression": 0.923,
            },
            "metrics": [
...
def build_china_cac_labeling() -> None:
    ...
    system = pkg.add_subject(
        ...
        name="SmartChat CN",
        version="4.0.0",
        provider="SinoAI Technology Co., Ltd.",
        risk_classification="gpai",
    ...
    pkg.record(
        "transparency_marking",
        provisions=["cac-implicit-metadata", "cac-watermark"],
        payload={
            "modality": "text",
            ...
            "implicit_label_fields": {
                "provider_id": "SINOAI-2025-00142",
                "content_id": "GENTEXT-20260301-874921",
                "content_type": "text",
                "reference_number": "CAC-REF-2026-874921",
            },
        },
        obligation_role="provider",
        entity_refs=refs,
        lifecycle_phase="deployment",
        collector={"name": "smartchat-marking-service", "version": "4.0.0"},
        timestamp=TS_RECENT,
    )
    ...
    pkg.record(
        ...
        payload={
            ...
            "implicit_label_fields": {
                "provider_id": "SINOAI-2025-00142",
                "content_id": "GENIMG-20260301-100234",
                "content_type": "image",
                "reference_number": "CAC-REF-2026-100234",
            },
        },
        obligation_role="provider",
        entity_refs=refs,
    ...
    pkg.record(
        "disclosure_labeling",
        provisions=["cac-explicit-label"],
        payload={
            "disclosure_subtype": "public_interest_text",
            "label_type": "ai_generated_content_notice",
            "presentation": "text_superscript",
            "locale": "zh-CN",
        ...
        entity_refs=refs,
        lifecycle_phase="deployment",
        collector={"name": "smartchat-labeling-service", "version": "4.0.0"},
        timestamp=TS_RECENT,
    )

    pkg.record(
        "event_log",
        provisions=["cac-log-retention"],
        payload={
            "event_type": "disclosure",
            "correlation_id": "DIS-CN-2026-03-01-00047",
            "session_id": "CHAT-CN-20260301-00012",
            "label_exception": True,
            "system_state": "operational",
            "retention_start_event": "record_creation",
        },
        obligation_role="provider",
        entity_refs=refs,
        lifecycle_phase="deployment",
    ...
    pkg.record(
        ...
        payload={
            "event_type": "inference",
            "correlation_id": "INF-CN-2026-03-01-00048",
            "session_id": "CHAT-CN-20260301-00013",
            "label_exception": False,
            "system_state": "operational",
        },
        obligation_role="provider",
        entity_refs=refs,
...
def build_us_federal_governance() -> None:
    ...
    system = pkg.add_subject(
        "ai_system",
        name="FedBenefitsAssist",
        version="2.1.0",
        provider="Federal Benefits Administration",
        risk_classification="high-risk",
        modalities=["text", "multimodal"],
        lifecycle_phase="deployment",
    )
    ...
    # Attachment
    pkg.add_attachment(
        "governance-board-charter-fba.pdf", b"[Federal Benefits Administration AI Governance Board Charter Document]"
    )
    ...
    pkg.record(
        ...
        payload={
            ...
            "impact_classification_rationale": (
                "System influences access to federal benefits. Classified as rights-impacting per OMB M-24-10."
            ),
        ...
        attachments=[
            {
                "path": "artifacts/governance-board-charter-fba.pdf",
                "media_type": "application/pdf",
    ...
    pkg.record(
        ...
        payload={
            ...
            "description": (
                "Algorithmic bias in benefits eligibility determination may disproportionately affect protected classes"
            ),
            "category": "fairness",
    ...
    pkg.record(
        ...
        payload={
            ...
            "results_summary": (
                "FedBenefitsAssist v2.1 demonstrates strong accuracy and fairness across all tested subgroups."
            ),
        },
        obligation_role="provider",
        entity_refs=refs,
    ...
    pkg.record(
        ...
        payload={
            ...
            "treatment_type": "mitigate",
            "control_description": (
                "Multi-layered bias mitigation: pre-processing demographic parity sampling, in-processing adversarial "
                "debiasing, post-processing threshold calibration, and mandatory human review for all denials"
            ),
            "implementation_status": "verified",
...
def build_multi_subject_composed() -> None:
    ...
    system = pkg.add_subject(
        "ai_system",
        name="InsightAnalytics Platform",
        version="4.0.0",
        provider="DataInsight Corp",
        risk_classification="high-risk",
        modalities=["text", "multimodal"],
        lifecycle_phase="deployment",
    )
    ...
    shared_db = pkg.add_component(
        ...
        "database",
        version="3.0.0",
        subject_refs=[system.subject_id, model.subject_id],
        provider="DataInsight Corp",
    )
    ...
    pkg.record(
        ...
        provisions=["article-9"],
        payload={
            "risk_id": "SYS-RISK-001",
            "description": "Hallucination risk in RAG pipeline",
            "category": "reliability",
            "review_type": "management_review",
        ...
        obligation_role="provider",
        entity_refs=refs_both,
        lifecycle_phase="deployment",
        collector=DEFAULT_COLLECTOR,
        attachments=[
            {
                "path": "artifacts/pmp-insight.pdf",
                "media_type": "application/pdf",
                "attachment_type": "post_market_monitoring_plan",
            }
        ],
    ...
    pkg.record(
        "data_provenance",
        provisions=["article-10", "article-53"],
        payload={
            "acquisition_method": "licensed",
            "source_uri": "https://enterprise-data-consortium.example.com/corpus/2025/",
    ...
    pkg.record(
        ...
        provisions=["article-10"],
        payload={
            "dataset_name": "Enterprise Corpus 2025",
            "dataset_version": "1.0",
            "source": "Enterprise Data Consortium",
            "modality": "text",
            "purpose": "training",
            "size": {"records": 50000000, "size_gb": 120.0},
    ...
    pkg.record(
        ...
        payload={
            ...
            "reserved_works_removed": 0,
            "opt_out_signals_detected": 0,
    ...
    pkg.record(
        "evaluation_report",
        provisions=["article-11", "article-53"],
        payload={
            "methodology": (
                "End-to-end evaluation of InsightAnalytics Platform including underlying InsightBERT-Large model"
            ),
            "metrics": [
...
def build_eu_test_vectors() -> None:
    ...
    def art9_pass():
        ...
        s = pkg.add_subject(
            "ai_system",
            name="MinimalRiskSystem",
            risk_classification="high-risk",
            modalities=["text"],
            provider="TestProvider Inc",
        )
        ...
        pkg.record(
            ...
            payload={
                "risk_id": "R-001",
                "description": "Bias risk in scoring",
                "category": "fairness",
                "review_type": "management_review",
                "review_date": "2026-02-01",
                "review_attendees": ["CTO"],
    ...
    def art9_variant_pass():
        ...
        pkg.record(
            ...
            payload={
                "risk_id": "R-010",
                "description": "Data drift risk",
                "category": "reliability",
                "review_type": "management_review",
                "review_date": "2026-01-15",
                "review_attendees": ["VP Eng", "CISO"],
                "review_decisions": ["Enhance monitoring"],
    ...
    def art53_pass():
        ...
        pkg = Package(producer={"name": "acef-test-vector-builder", "version": "1.0.0"})
        s = pkg.add_subject(
            "ai_model", name="GPAIModel", risk_classification="gpai", modalities=["text"], provider="TestProvider Inc"
        )
        d = pkg.add_dataset("TrainingData", source_type="scraped", modality="text", subject_refs=[s.subject_id])
        pkg.add_profile("eu-ai-act-2024", provisions=["article-53"])
        refs = {"subject_refs": [s.subject_id]}
        ...
        pkg.record(
            ...
            payload={
                "acquisition_method": "scraped",
                "source_uri": "https://data.example.com/corpus",
                "source_type": "scraped",
            },
            entity_refs=refs_ds,
            obligation_role="provider",
    ...
    def multi_subject():
        ...
        pkg.record(
            ...
            payload={
                ...
                "review_attendees": ["CTO"],
                "review_decisions": ["Mitigate"],
                "review_signatories": ["CTO"],
            },
            entity_refs={"subject_refs": [sys_subj.subject_id]},
            obligation_role="provider",
            lifecycle_phase="monitoring",
            collector=TV_COLLECTOR,
            attachments=[
                {
                    "path": "artifacts/pmp-ms.pdf",
                    "media_type": "application/pdf",
                    "attachment_type": "post_market_monitoring_plan",
                }
            ],
        ...
        pkg.record(
            ...
            payload={
                "acquisition_method": "licensed",
                "source_uri": "https://data.example.com",
                "source_type": "licensed",
            },
            entity_refs={"subject_refs": [mdl_subj.subject_id]},
            obligation_role="provider",
            lifecycle_phase="development",
            collector=TV_COLLECTOR,
            timestamp=TS_RECENT,
...
def build_china_cac_test_vectors() -> None:
    ...
    def cac_pass():
        ...
        pkg.record(
            "transparency_marking",
            provisions=["cac-implicit-metadata"],
            payload={
                "modality": "text",
                "marking_scheme_id": "cn-cac-implicit-label-2025",
                "scheme_version": "1.0",
        ...
        pkg.record(
            ...
            payload={
                "disclosure_subtype": "public_interest_text",
                "label_type": "ai_generated_notice",
                "presentation": "text_superscript",
                "locale": "zh-CN",
                "disclosure_text": "\u672c\u5185\u5bb9\u7531\u4eba\u5de5\u667a\u80fd\u751f\u6210",
                "first_exposure_timestamp": "2026-03-01T08:00:00Z",
    ...
    def cac_exception_pass():
        ...
        pkg.record(
            ...
            payload={
                ...
                "watermark_applied": True,
                "watermark_family": "spectral_embedding",
                "verification_method_ref": "artifacts/cn-verify.json",
                "jurisdiction": "CN",
            },
            entity_refs=refs,
            obligation_role="provider",
...
def build_nist_test_vectors() -> None:
    ...
    def nist_pass():
        ...
        d = pkg.add_dataset("NISTTrainData", source_type="licensed", modality="tabular", subject_refs=[s.subject_id])
        pkg.add_profile(
            "nist-ai-rmf-1.0", provisions=["govern-1", "map-1", "map-2", "measure-1", "manage-1", "manage-4"]
        )
        refs = {"subject_refs": [s.subject_id]}
        refs_ds = {"subject_refs": [s.subject_id], "dataset_refs": [d.dataset_id]}
        ...
        pkg.record(
            ...
            payload={
                "policy_type": "ai_governance_policy",
                "title": "NIST RMF Governance Framework",
        ...
        pkg.record(
            ...
            provisions=["map-2"],
            payload={
                "dataset_name": "NISTTrainData",
                "dataset_version": "1.0",
                "source": "Internal data lake",
                "modality": "tabular",
            },
            entity_refs=refs_ds,
            obligation_role="provider",
        ...
        pkg.record(
            ...
            payload={
                "acquisition_method": "licensed",
                "source_uri": "https://internal.example.com/data",
            },
            entity_refs=refs_ds,
            obligation_role="provider",
            lifecycle_phase="development",
...
def build_omb_test_vectors() -> None:
    ...
    def omb_pass():
        ...
        s = pkg.add_subject(
            "ai_system",
            name="FederalBenefitsClassifier",
            risk_classification="high-risk",
            modalities=["text"],
        ...
        pkg.add_profile(
            "us-omb-m-24-10", provisions=["omb-inventory", "omb-caio", "omb-rights-safety", "omb-governance"]
        )
        refs = {"subject_refs": [s.subject_id]}
        refs_actor = {"subject_refs": [s.subject_id], "actor_refs": [oversight_actor.actor_id]}

        pkg.add_attachment("ai-governance-framework.pdf", b"[AI Governance Framework Document - Federal Agency]")

        # governance_policy: ai_use_case_inventory_entry variant with all required fields
        pkg.record(
            "governance_policy",
            provisions=["omb-inventory", "omb-caio", "omb-rights-safety"],
            payload={
                "variant": "ai_use_case_inventory_entry",
                "policy_type": "ai_use_case_inventory",
                "title": "FederalBenefitsClassifier AI Use-Case Inventory Entry",
                "system_purpose": (
                    "Automated classification of federal benefit applications to route to appropriate review "
                    "teams based on complexity and eligibility criteria"
                ),
                "decision_influence": "advisory",
                "rights_safety_impact": "rights_impacting",
                "impact_classification_rationale": (
                    "System influences decisions about federal benefit eligibility which has material impact on "
                    "individuals access to government services and financial assistance"
                ),
        ...
        pkg.record(
            ...
            payload={
                ...
                "rights_safety_impact": "rights_impacting",
                "impact_classification_rationale": (
                    "Agency governance framework applies to all AI systems including those with rights-impacting "
                    "classification per OMB M-24-10"
                ),
            },
            entity_refs=refs,
            obligation_role="provider",
            ...
            attachments=[
                {
                    "path": "artifacts/ai-governance-framework.pdf",
                    "media_type": "application/pdf",
...
def build_gpai_cop_test_vectors() -> None:
    ...
    def gpai_cop_pass():
        ...
        pkg.add_profile(
            ...
            provisions=["gpai-transparency-1", "gpai-transparency-2", "gpai-copyright-1", "gpai-copyright-2"],
        )
        refs = {"subject_refs": [s.subject_id]}
        refs_ds = {"subject_refs": [s.subject_id], "dataset_refs": [d.dataset_id]}
        ...
        pkg.record(
            ...
            payload={
                ...
                "removal_process": (
                    "Automated weekly re-crawl of robots.txt and TDMRep signals with batch removal from training "
                    "pipeline"
                ),
            },
            entity_refs=refs,
        ...
        pkg.record(
            ...
            payload={
                ...
                "restrictions": [
                    "No verbatim reproduction in model outputs",
                    "Attribution required in training data summaries",
                ],
            },
            entity_refs=refs_lic_ds,
            obligation_role="provider",
            lifecycle_phase="development",
            collector=TV_COLLECTOR,
            timestamp=TS_RECENT,
