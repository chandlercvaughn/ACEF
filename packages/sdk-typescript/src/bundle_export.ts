...
function strictRfc3339ToEpochSeconds(ts: string): number | null {
    ...
    if (!m) return null;
    // Require the WHOLE input to match. The explicit ``m[0] === ts`` full-match check makes
    // the intent unambiguous regardless of any JS ``$``-anchor / line-terminator subtlety, so
    // a stray trailing character (e.g. a trailing newline) can never slip a value past the
    // Python exporter's rejection (roborev on 41c41ed).
