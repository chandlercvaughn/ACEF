...
1. **JSON canonicalization:** All JSON files in the hash domain MUST be serialized using [RFC 8785 (JCS)](https://www.rfc-editor.org/rfc/rfc8785) for deterministic output before hashing. Implementations MUST verify that JSON is valid UTF-8 with NFC normalization and no BOM (U+FEFF). **Integer domain (I-JSON):** every JSON number used as an integer in the hash domain MUST lie within the RFC 7493 (I-JSON) safe-integer range −(2^53 − 1) … 2^53 − 1 (i.e. |n| ≤ 9007199254740991). This is a **record-authoring constraint**, not merely a hash-time check: producers MUST reject an out-of-range integer at record construction. An out-of-domain integer that nonetheless reaches canonicalization is a FATAL canonicalization fault (ACEF-051), because RFC 8785 number serialization is only defined within the I-JSON range and two implementations could otherwise serialize it differently.
...
- **The DEFLATE-compressed body is explicitly OUTSIDE the conformance surface and OUTSIDE the hash domain.** DEFLATE output is a function of the compressor implementation and version (zlib memory level, strategy, version) and is NOT standardized by RFC 1952, so two conforming implementations MAY emit byte-different `.acef.tar.gz` files for the same logical bundle. Byte-identical *archives* are therefore NOT a conformance requirement, and prior revisions' "byte-identical archives across runtimes" claim is withdrawn as unachievable across DEFLATE implementations.
- **Bundle identity and conformance are defined over the UNPACKED bundle.** The canonical identity is the **bundle digest** — SHA-256 of the RFC 8785-canonicalized `content-hashes.json` (§3.1.1) — recomputed from the hash domain. The byte-reproducible deterministic artifact is the **UNCOMPRESSED tar member stream** (the fixed member order, mtime, owner/group `0/0`, and `0644`/`0755` permissions above), not the gzip wrapper. A conforming validator MUST verify a received bundle by unpacking it and recomputing the hash domain (steps a–f), and MUST NOT determine conformance by byte-comparing `.acef.tar.gz` archives. The `.acef.tar.gz` is a transport container; the bundle digest is the identity.
...
| **Bundle identity (over the unpacked tree)** | Recomputing the bundle digest (SHA-256 of RFC 8785-canonicalized `content-hashes.json`) and the UNCOMPRESSED tar member stream (fixed order, mtime, owner `0/0`, perms `0644`/`0755`) is byte-reproducible across exporters. The gzip/DEFLATE wrapper is a transport container and is NOT byte-compared for conformance (§3.1.3). |
