...
**Deterministic resource bound (normative).** The `regex` operator's outcome MUST be **platform-independent**: two conformant validators evaluating the same rule against the same record MUST reach the same result (or the same ACEF-045). Therefore the resource bound that guards catastrophic backtracking MUST be **deterministic**, NOT a wall-clock timeout (a timeout makes the verdict depend on platform/CPU/thread — e.g. a SIGALRM that fires only on the Unix main thread). A conformant validator MUST: (a) reject a pattern longer than **1024** characters and an input longer than **1 000 000** characters as ACEF-045 (these caps are normative and fixed, NOT implementation-defined, so a pattern/input near a boundary gets the same verdict from every validator); and (b) reject, as ACEF-045, a pattern exhibiting either catastrophic-backtracking signature — an unbounded-quantified group (`(…)*`, `(…)+`, `(…){n,}`) whose body either contains an unbounded quantifier (the **nested-quantifier** class, e.g. `(a+)+`, `(a*)*`, `(.*)+`) OR a top-level alternation (the **alternation-overlap** class, e.g. `(a|aa)+`, `(a|a)*`) — *before* matching. This static rejection is deterministic and identical across platforms. (The rejection is conservative — it also rejects some safe quantified alternations like `(a|b)+`; this is an accepted trade for determinism + ReDoS safety given DSL patterns are short rule-level matchers.)
