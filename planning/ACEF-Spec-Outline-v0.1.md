...
1. **JSON canonicalization:** All JSON files in the hash domain MUST be serialized using [RFC 8785 (JCS)](https://www.rfc-editor.org/rfc/rfc8785) for deterministic output before hashing. Implementations MUST verify that JSON is valid UTF-8 with NFC normalization and no BOM (U+FEFF). **Unique member names (I-JSON):** every JSON object in **any** hash-domain file MUST have **unique** member names (RFC 7493 §2.3). A validator MUST **reject** a hash-domain `.json`/`.jsonl` containing a duplicate object member name as a FATAL canonicalization fault (ACEF-051) — it MUST NOT silently apply last-wins (or first-wins) de-duplication. This is required for cross-implementation determinism and integrity: a permissive parser (`json.loads` / `JSON.parse` are last-wins) would let `{"a":1,"a":2}` hash identically to `{"a":2}`, so a signed manifest could carry a duplicate member that a first-wins or I-JSON-strict verifier resolves to a different value than the producer intended, while the bundle digest stays unchanged. **Integer domain (I-JSON):** every JSON number used as an integer in **any** hash-domain file — record payloads/envelopes, `acef-manifest.json`, and JSON `artifacts/` — MUST lie within the RFC 7493 (I-JSON) safe-integer range −(2^53 − 1) … 2^53 − 1 (i.e. |n| ≤ 9007199254740991). This is an **authoring/export constraint** across the whole hash domain, not merely a hash-time check: a conforming producer MUST reject an out-of-range integer when it authors a record, assembles the manifest, or admits a JSON artifact. An out-of-domain integer that nonetheless reaches canonicalization is a FATAL canonicalization fault (ACEF-051) for any hash-domain file, because RFC 8785 number serialization is only defined within the I-JSON range and two implementations could otherwise serialize it differently.
...
| `exists_where_any` | `record_type`: string, `fields`: JSON Pointer[] (non-empty), `op`: comparison, `value`: any, `min_count`: int | At least `min_count` records exist where **any** pointer in `fields` resolves and satisfies the comparison. A record satisfying several listed pointers counts ONCE — the disjunction selects records, it does not multiply them. Every pointer and the `op` are validated up front, so a malformed rule raises (ACEF-043/ACEF-046) even when zero records match. An empty `fields` list is a malformed rule. Required where one obligation may be evidenced on either of two representations: retention, for instance, is stated either on the record envelope (`/retention/min_retention_days`) or in the `logging_spec` payload (`/payload/retention_policy_summary/min_days`), and a single-pointer rule would fail records that legitimately use only one. |
...
| `record_attested` | `record_type`: string, `min_count`: int (default 1) | At least `min_count` records of the given type have a non-null `attestation` block with a valid JWS signature (v1 restricts attestation to JWS only; C2PA record attestation is deferred to a future profile). Verification: extract the fields listed in `signed_fields`, canonicalize via RFC 8785, verify the detached JWS. **Trust anchoring (consistency clause):** this verification uses the SAME trust configuration as §3.1.3 integrity verification within one evaluation. If the validator is run with configured x5c trust anchors, only an **anchored** `x5c` chain counts: a chain that does not terminate at a configured anchor MUST NOT satisfy `record_attested` (an attestation whose `x5c` would be rejected as a bundle signature, ACEF-012, MUST NOT count in the same anchored run), AND a `jwk`-only attestation (no `x5c`) is `self-attested` — an attacker can re-sign a tampered record with their own auto-embedded key — so it likewise MUST NOT satisfy `record_attested` under configured anchors. When no anchors are configured, the attestation is `self-attested` (the JWS and any x5c validity window are still verified; no external anchoring is enforced) and DOES count — identical to the bundle-signature self-attested posture (§3.1.3, Appendix D.3) and symmetric with the `harness_attestation` signature check (§3.6). x5c certificate validity is checked against `manifest.metadata.timestamp`, not wall-clock. |

**Empty-set semantics (normative):** If a rule's scope or `record_type` filter matches zero records:
- **Existential operators** (`has_record_type`, `exists_where`, `exists_where_any`, `attachment_exists`, `attachment_kind_exists`, `record_attested`, `bundle_signed`) evaluate to `failed` if `min_count > 0` (for `bundle_signed`, if `min_signatures > 0`). These require a witness to exist — a matching record/attachment, a *verified* record attestation, or a *verified* bundle signature. (`bundle_signed`'s "empty set" is the bundle's **signatures** set, not a record set: zero verified signatures → `failed`; it is unaffected by `record_type`.)
- **Universal operators** (`field_present`, `field_value`, `entity_linked`, `evidence_freshness`) evaluate to `passed` (vacuous truth). These assert a property of every in-scope record and are trivially satisfied when no records exist.
...
4. **Bounded/nullable group-repetition** — a group repeated **more than once** — by an unbounded quantifier (`*`, `+`, `{n,}`) **OR a *bounded* counted quantifier whose upper bound is ≥ 2** (`{n}` with n ≥ 2, `{n,m}` with m ≥ 2) — whose body is **nullable** (can match the empty string, e.g. `a?`, `a*`, `a{0,m}`, or a nested group that is itself nullable), contains a top-level **alternation**, or contains a nested unbounded quantifier (e.g. `(a?){28}a{28}`, `(a|a){24}b`, `(.?){24}x`, `(a{0,n}){0,n}$`, and `(a?)+`). A **bounded** outer quantifier is NOT safe: a counted repetition of an ambiguous body still has exponentially many match factorizations, and the offending pattern is short (well under the 1024 cap), so neither length cap mitigates it. This is the signature a check defined only over *unbounded* quantifiers (1–3) misses.

This static rejection is deterministic and identical across platforms. (The rejection is conservative — it also rejects some safe constructs, e.g. the quadratic-but-tolerable `a.*b.*c`, two adjacent quantified groups, quantified alternations like `(a|b)+`, or a small nullable counted repetition like `(a?){2}` (harmless at n=2 but conservatively rejected because its body is nullable); this is an accepted trade for determinism + ReDoS safety given DSL patterns are short rule-level matchers, and a future linear-time engine would accept the safe ones precisely. A counted repetition whose body is a **non-nullable, non-alternating, non-nested** fixed sequence — e.g. `(ab){5}`, `(\d{4}){2}` — is NOT flagged, as it matches in linear time.)
...
| `ACEF-034` | Profile | `error` | Retention determination missing or internally inconsistent on a provision — a `retention_years` figure with no `retention` provenance block; a `fixed_period`/`minimum_floor` kind with no `period`; a `none_stated`/`not_assessed` kind carrying one; a `kind`/`source` disagreement over whether a determination exists; an `inferred` source whose `basis` omits the literal token `INFERRED`; a `cited` source with an empty `normative_text_ref` or an `INFERRED` basis; an empty `basis`; or a `retention_years` scalar disagreeing with the structured block. A retention period asserted with no recorded source is not auditable: a consumer cannot tell a figure the instrument states from one a profile author inferred, or from one nobody researched. |
| `ACEF-035` | Profile | `info` | Applicability is INDETERMINATE: the provision commences on two or more dates keyed to a classification of the subject that the bundle cannot express, and `evaluation_instant` falls between the earliest and latest of them. The reported outcome uses the EARLIEST limb and is a conservative projection, not a determination. Both limbs are recorded on the provision's `tiered_requirements.adoption`. Distinct from `ACEF-032` (not yet effective under ANY limb) as XACML `Indeterminate` is distinct from `NotApplicable`. |
| `ACEF-036` | Profile | `info` | A retention obligation stated in calendar months is screened by a fixed day-count threshold, so exact calendar satisfaction was NOT verified. Calendar months have no fixed length (six months spans 181-184 days) and no §3.5 operator resolves a duration against a per-record anchor, so the threshold is set to the LOWER bound of the stated period: it never rejects a compliant record but cannot confirm one. The authoritative obligation is the provision's `retention.period` and `retention.anchor_event`. |
...
**Default when the caller omits `evaluation_instant` (normative).** For a **structurally valid** Evidence Bundle that a validator actually accepts and evaluates, the validator MUST be reproducible: it MUST NOT silently substitute wall-clock time as the `evaluation_instant` that gates `evidence_freshness`/`effective_date`. When the calling API does not pin an explicit `evaluation_instant`, a conformant validator therefore derives it **deterministically from the bundle** — the reference implementation uses the Evidence Bundle's `metadata.timestamp` — and records the derived value in the Assessment Bundle. **Scope of the guarantee.** `metadata.timestamp` is a REQUIRED, ISO-8601 `string` manifest field (§3.1.1 manifest schema). A bundle that lacks a usable one — a missing/malformed/non-object `acef-manifest.json`, or a `metadata.timestamp` that is absent, non-string, or not ISO-8601 — is therefore **structurally invalid**: its Assessment Bundle carries a **FATAL** error (ACEF-002/ACEF-014/ACEF-050) and the bundle is **rejected**. The reference validator, for a *malformed/non-object/missing manifest*, short-circuits to a structural-error stub (no further phases run); for a *schema-invalid `metadata.timestamp`* on an otherwise-parseable manifest it completes the remaining phases (to **collect all errors** — the §3.6 "collect every error in a phase" contract) using a best-effort `evaluation_instant` whose value depends on the defect: an **absent or non-string** `metadata.timestamp` falls back to a **wall-clock** value, while a **present-but-non-ISO string** is recorded **verbatim** (the schema-invalid string itself). Either way the resulting `evidence_freshness`/`effective_date` diagnostics are **NON-AUTHORITATIVE**: the assessment already carries a FATAL `ACEF-002` timestamp schema error (reported at `/metadata/timestamp` for a wrong-typed or non-ISO value, or at the parent `/metadata` when the required property is absent) and is rejected, and **no accepted (non-fatal) assessment ever depends on a wall-clock or otherwise non-deterministic instant**. The reproducibility guarantee thus binds exactly the accepted/evaluated path; a non-deterministic fallback instant only ever appears inside an already-rejected assessment. **Security caveat (normative):** `metadata.timestamp` is **producer-controlled**, so an evaluation_instant derived from it is producer-controlled too, and a backdated/forward-dated `metadata.timestamp` moves the `evidence_freshness` and `effective_date` gates accordingly. A consumer who needs freshness/effective-date gating to be independent of producer-supplied timestamps — e.g. a regulator assessing at filing time — **MUST** pass an explicit `evaluation_instant` (the deployment/check time). This is the same limitation disclosed as an explicit **non-goal** in Appendix D.4 (trusted-timestamping is future work): absent an external trusted-timestamp profile, evaluation-time freshness is governed only by producer-controlled record timestamps, not by a third party.
...
| **Template evaluation** | Reference validator produces identical `provision_outcome`/`structural_errors` from identical Evidence Bundles + templates + `evaluation_instant` (the §6.6 reproducibility scope — NOT byte-identical whole Assessment Bundles: an unpinned assessment additionally carries a wall-clock `timestamp` and a random `assessment_id`) |
