...
**Format version covered:** ACEF Core **v1**. Schema conventions ship in two registries — `v1.0` (frozen) and `v1.1` (additive). Valid `manifest.versioning.core_version` values are semver in the **`1.0.x` and `1.1.x`** ranges (reference bundles use `1.0.0` and `1.1.0`); the schema set is selected by the MAJOR.MINOR pair per §6.2 (`1.0.x` → `acef-conventions/v1/`, `1.1.x` → `acef-conventions/v1.1/`). A `1.0.x` bundle MUST validate identically under this revision and under any pre-0.4 revision.
